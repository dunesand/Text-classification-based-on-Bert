...
path = "F:/MachineLearning/DATA/bert_model/vocab/cnews.train.txt"
tokenizer = BertTokenizer("F:/MachineLearning/DATA/bert_model/chinese_roberta_wwm_ext_pytorch/vocab.txt")
...
with open(path, encoding='utf-8') as f:
    for i, l in tqdm(enumerate(f)): 
        ...
        if len(ids) < pad_size:
            ...
            ids = ids + [0] * (pad_size - len(ids))    #padding部分初始化为0
        else:
...
#         print(len(ids), len(masks), len(types)) 
        ...
        label.append(y)
    trans={'体育':1,'娱乐':2,'家居':3,'房产':4,'教育':5,'时尚':6,'时政':7,'游戏':8,'科技':9,'财经':10}
    label=[trans[i] if str(i) in trans else i for i in label]

# 随机打乱索引
random_order = list(range(len(input_ids)))
np.random.seed(2020)   # 设置seed()值，保证每次产生的随机数相同
...
class Model(nn.Module):
    def __init__(self):
        ...
        self.bert = BertModel.from_pretrained("F:/MachineLearning/DATA/bert_model/chinese_roberta_wwm_ext_pytorch/")  
        for param in self.bert.parameters():
            param.requires_grad = True  # 训练过程中参数更新
        self.fc = nn.Linear(768, 10)   # 全连接层

    def forward(self, x):
        context = x[1]  # 输入的句子   (ids, seq_len, mask)
        types = x[0]
